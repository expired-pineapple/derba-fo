...
const dispatch = async () => {
  try {
    ...
    await store.dispatch("fetchTrailerThirdParty", id)
    ...
    data.value = store.getters.trailerThirdParties
    insurance.value = store.getters.trailerThirdParty
...
const search = () => {
  ...
  const filteredInsurances = data.value.filter(item => {
    return item.trlThirdInsNo.toLowerCase().includes(search.toLowerCase())
  })

  if(!search){
    return data.value = store.getters.trailerThirdParties
  }
...
const edit = async item => {
  ...
  router.push({ name: 'edit-third-party', params: { id: item.id } })
  await store.dispatch('fetchTrailerThirdParty', item.id)
  insurance.value = store.getters.trailerThirdParty
  console.log('Insurance:', insurance.value)
}
...
const submitForm = async () => {
  ...
  try {
    await store.dispatch('updateTrailerThirdParty', insurance.value)
    await dispatch()
    if(!error.value){
...
const deleteItem = async item => {
  ...
  await store.dispatch('deleteTrailerThirdParty', item.id)
  await dispatch()
  router.push('/trailer-third-party')
}
            ...
            >
              <div class="d-flex mt-4">
                ...
                <div>
                  <div class="d-flex">
                    <div class="info">
                      <span class="font-weight-semibold me-2 mb-4">Insurance: {{ item.trlThirdInsNo }}</span>
                    </div>
                    <div class="icon">
                      <VBtn
                        icon="mdi-pencil"
                  ...
                  <VForm @submit.prevent="submitForm">
                    <VContainer>
                      <VRow>
                        ...
                        >
                          <VTextField
                            v-model="insurance.trlThirdInsNo"
                        ...
                        >
                          <VTextField
                            v-model="insurance.trlThirdIssuanceDate"
                            label="Issuance Date"
                            required
                            type="date"
                          />
                        ...
                        >
                          <VTextField
                            v-model="insurance.trlThirdValidationDate"
                            label="Valid Date"
                            required
                            type="date"
                          />
                        ...
                        >
                          <VTextField
                            v-model="insurance.trlThirdPolicyNo"
                        ...
                        <VCol cols="12">
                          <VSwitch
                            v-model="insurance.trlThirdActive"
                            label="Active"
                          />
                        </VCol>
                      </VRow>
